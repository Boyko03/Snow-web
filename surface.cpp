	...
	m_Flags(OWNER)
{
	...
	m_Buffer = new Pixel[(unsigned int)a_Width * (unsigned int)a_Height * sizeof(Pixel)];
#else
	m_Buffer = (Pixel*)MALLOC64((unsigned int)a_Width * (unsigned int)a_Height * sizeof(Pixel));
#endif
}
...
{
	SDL_Surface* image = IMG_Load(a_File);
	m_Width = m_Pitch = image->w;
	m_Height = image->h;

#ifdef __EMSCRIPTEN__
	m_Buffer = new Pixel[m_Pitch * m_Height * sizeof(Pixel)];
#else
	...
	if (m_Buffer) {
		...
		memcpy(m_Buffer, image->pixels, m_Pitch * m_Height * sizeof(Pixel));
