...
void main_loop()
{
	...
	if (firstframe) {
		...
		frameBuffer = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, ScreenWidth, ScreenHeight);
	}
#ifdef ADVANCEDGL
	swap();
...
#else
	Timer::Get().Tick();
	float elapsedTime = Timer::Get().ElapsedMilliSeconds();
#endif
